...
外交・安全保障・紛争／戦争・世界情勢（国際政治）を事実ベース・出典付きで毎日届けるサイト。日本の外交・安保も対象。Codex routine が定期実行で記事を追加する。国内政治の内輪の話題は扱わない。
...
## 記事を追加するときは必ず
1. **`.agents/skills/news-collection/`** の手順・編集方針に従う（中立性・出典・帰属表現・記事構成のルール）。
2. **`.agents/skills/article-registry/`** で重複チェック → 投稿後に台帳登録。
...
## SEO / LLMO
- 最適化の方針・維持手順は **`.agents/skills/seo-llmo/`** に集約。記事を書くたび「結論先出し・数値＋出典・明確な見出し・強い `summary`」を満たす。
