...
3. **機械判定（決定的・スクリプト）**:
   ```bash
   bun run .agents/skills/article-registry/scripts/registry.ts check \
...
5. **記事生成 → 台帳に追記**:
   ```bash
   bun run .agents/skills/article-registry/scripts/registry.ts add \
...
## 公開後の通知（このサイトの記事作成スキルと併用）
記事を push して公開したら、**Slack コネクタ**で公開記事の**タイトル＋URL**を `#claude-code` に通知する（各サイトの記事作成スキルに通知の詳細・URL 形式・例を記載）。記事を1本作るごとの締めとして必ず行う。

## 終了条件
- [ ] 台帳をロードし機械判定を実行した
- [ ] `review` の候補を LLM が意味的に判定した
- [ ] 投稿する場合は台帳に追記し、記事と同一コミットに含めた
- [ ] 公開後、Slack コネクタで記事のタイトル＋URL を `#claude-code` に通知した（各サイトの記事作成スキル参照）
